...
#' @examples
#'
#' sc112 <- get_SC112(list(lake = "ON", year = 2012))
#'
#' sc112 <- get_SC112(list(prj_cd = "LHA_SC08_120"))
#'
#' sc112 <- get_SC112(list(prj_cd = "LHA_SC08_120"), show_id = TRUE)
get_SC112 <- function(filter_list = list(), show_id = FALSE, to_upper = TRUE, record_count = FALSE) {
  recursive <- ifelse(length(filter_list) == 0, FALSE, TRUE)
  query_string <- build_query_string(filter_list)
  check_filters("sc112", filter_list, api_app = "creels")
  my_url <- sprintf(
    "%s/sc112/%s",
    get_sc_portal_root(),
    query_string
