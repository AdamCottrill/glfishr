#' Get FN121limno - Net set limnological data from FN_Portal API
#'
...
#' information like disolved oxygen at the surface or at the gear at
...
#' several attributes of the limnological data as well as atttributes
...
#' protocol, etc.  The limnogical data can be joined back to the
...
#'
#' filter_list <- list(prj_cd = "LHA_IA14_801")
#' fn121limno <- get_FN121limno(filter_list)
#'
#' # pass the same fitler to our FN121 to get the associated net sets:
#'
#' fn121 <- get_FN121(filter_list)
#' joined <- join_fn_fields(fn121, fn121limno)
#' # net sets are now associated with their limnlogical data:
#' head(joined)
get_FN121limno <- function(filter_list = list(), show_id = FALSE, to_upper = TRUE) {
